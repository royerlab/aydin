"""Demo script for 2D harmonic filter denoising with auto-calibration."""
...
from aydin.it.classic_denoisers.harmonic import calibrate_denoise_harmonic
...
def demo_harmonic(image, display=True):
    """Denoise a 2D image using auto-calibrated harmonic filtering."""
    ...
    start = time.time()
    function, parameters, memreq = calibrate_denoise_harmonic(noisy)
    ...
    print("harmonic denoised:", psnr_denoised, ssim_denoised)
    ...
    if display:
        ...
        plt.savefig(os.path.join(_DEMO_RESULTS, 'classic_harmonic_2D.png'))
...
if __name__ == "__main__":
    ...
    demo_harmonic(characters())
    demo_harmonic(pollen())
    demo_harmonic(lizard())
    demo_harmonic(dots())
    demo_harmonic(camera())
    demo_harmonic(newyork())
