"""Demonstrate automatic blind-spot detection with CNN-based denoising.

This demo compares CNN (JINet) denoising with and without automatically
detected extended blind spots on a synthetically noised camera image with
correlated noise. Reports PSNR/SSIM metrics and saves comparison plots.
"""
...
def demo(image, name):
    """Compare CNN denoising with and without auto-detected blind spots.
    ...
    itwbs = ImageTranslatorCNNTorch(
        ...
        max_epochs=20,
        blind_spots=auto_detect_blindspots(noisy)[0],
    )
    ...
    plt.savefig(os.path.join(_DEMO_RESULTS, f'auto_blindspot_cnn_{name}.png'))
