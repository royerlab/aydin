"""Demo of Noise2Self LGBM denoising with uint8 feature quantization.

Trains an ``ImageTranslatorFGR`` with ``LGBMRegressor`` using features
computed in ``numpy.uint8`` precision to reduce memory usage, and
evaluates PSNR/SSIM on multiple standard test images.
"""
...
def demo(image, name):
    ...
    plt.savefig(os.path.join(_DEMO_RESULTS, f'n2s_lgbm_2D_{name}_uint8.png'))
