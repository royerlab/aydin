"""Tests for the n-dimensional patch extraction and reconstruction transform."""
...
def test_patch_transform():
    """Test patch extraction and reconstruction round-trips for odd and even sizes."""
    _round_trip(7, display=False)
    _round_trip(8, display=False)


def _round_trip(patch_size: int, display: bool):
    """Extract patches and reconstruct, verifying round-trip fidelity."""
    # Image:
    image = newyork()
