"""Tests for the text line-breaking utility."""
...
def test_line_breaks():
    """Test break_text wraps long text at the specified line width."""
    print("")
    print(break_text(_test_line_breaks_text, 80))


def test_strip_notgui_with_tag():
    """Test strip_notgui removes content after <notgui> tag."""
    text = "GUI visible part.\n<notgui>\nAttributes\n----------\nfoo : int\n"
    assert strip_notgui(text) == "GUI visible part."


def test_strip_notgui_without_tag():
    """Test strip_notgui returns full text when no tag is present."""
    text = "No tag here, just regular text."
    assert strip_notgui(text) == text


def test_strip_notgui_none():
    """Test strip_notgui returns empty string for None input."""
    assert strip_notgui(None) == ''


def test_strip_notgui_empty():
    """Test strip_notgui returns empty string for empty input."""
    assert strip_notgui('') == ''
