...
class ESAdam(Adam):
    ...
    **kwargs
    ...
    def __init__(self, params, start_noise_level=0.001, **kwargs):
        """Initialize the ESAdam optimizer.
    ...
    def state_dict(self):
        """Return optimizer state including ``step_counter``.

        Returns
        -------
        dict
            Optimizer state dict with ``step_counter`` added.
        """
        d = super().state_dict()
    ...
    def load_state_dict(self, state_dict):
        """Load optimizer state, restoring ``step_counter`` if present.

        Parameters
        ----------
        state_dict : dict
            Optimizer state dict, optionally containing ``step_counter``.
        """
