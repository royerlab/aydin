"""Tests for TensorFlow model architectures."""

import numpy as np

from aydin.nn.tf.models.jinet import JINetModel
from aydin.nn.tf.models.unet import UNetModel


def test_supervised_2D():
    np.random.seed(42)
    input_array = np.random.rand(1, 64, 64, 1).astype(np.float32)
    model2d = UNetModel(
        (64, 64, 1),
        nb_unet_levels=2,
        supervised=True,
        spacetime_ndim=2,
    )
    result = model2d.predict(input_array)
    assert result.shape == input_array.shape
    assert result.dtype == input_array.dtype
    # Verify model produces non-trivial output
    assert not np.allclose(result, 0)


def test_shiftconv_2D():
    np.random.seed(42)
    input_array = np.random.rand(1, 64, 64, 1).astype(np.float32)
    model2d = UNetModel(
        (64, 64, 1),
        nb_unet_levels=2,
        training_architecture='shiftconv',
        supervised=False,
        spacetime_ndim=2,
    )
    result = model2d.predict(input_array)
    assert result.shape == input_array.shape
    assert result.dtype == input_array.dtype
    # Verify model produces non-trivial output
    assert not np.allclose(result, 0)


def test_masking_2D():
    np.random.seed(42)
    input_array = np.random.rand(1, 64, 64, 1).astype(np.float32)
    model2d = UNetModel(
        (64, 64, 1),
        nb_unet_levels=2,
        supervised=False,
        spacetime_ndim=2,
    )
    result = model2d.predict([input_array, input_array])
    assert result.shape == input_array.shape
    assert result.dtype == input_array.dtype
    # Verify model produces non-trivial output
    assert not np.allclose(result, 0)


def test_jinet_2D():
    np.random.seed(42)
    input_array = np.random.rand(1, 64, 64, 1).astype(np.float32)
    model2d = JINetModel((64, 64, 1), spacetime_ndim=2)
    result = model2d.predict([input_array])
    assert result.shape == input_array.shape
    assert result.dtype == input_array.dtype
    # Verify model produces non-trivial output
    assert not np.allclose(result, 0)


def test_supervised_3D():
    np.random.seed(42)
    input_array = np.random.rand(1, 64, 64, 64, 1).astype(np.float32)
    model3d = UNetModel(
        (64, 64, 64, 1),
        nb_unet_levels=2,
        supervised=True,
        spacetime_ndim=3,
    )
    result = model3d.predict(input_array)
    assert result.shape == input_array.shape
    assert result.dtype == input_array.dtype
    # Verify model produces non-trivial output
    assert not np.allclose(result, 0)


def test_shiftconv_3D():
    np.random.seed(42)
    input_array = np.random.rand(1, 64, 64, 64, 1).astype(np.float32)
    model3d = UNetModel(
        (64, 64, 64, 1),
        nb_unet_levels=2,
        training_architecture='shiftconv',
        supervised=False,
        spacetime_ndim=3,
    )
    result = model3d.predict(input_array)
    assert result.shape == input_array.shape
    assert result.dtype == input_array.dtype
    # Verify model produces non-trivial output
    assert not np.allclose(result, 0)


def test_masking_3D():
    np.random.seed(42)
    input_array = np.random.rand(1, 64, 64, 64, 1).astype(np.float32)
    model3d = UNetModel(
        (64, 64, 64, 1),
        nb_unet_levels=2,
        supervised=False,
        spacetime_ndim=3,
    )
    result = model3d.predict([input_array, input_array])
    assert result.shape == input_array.shape
    assert result.dtype == input_array.dtype
    # Verify model produces non-trivial output
    assert not np.allclose(result, 0)


def test_various_masking_3D():
    np.random.seed(42)
    for i in [0, 4]:
        input_array = np.random.rand(1, 21 + i, 64, 64, 1).astype(np.float32)
        print(f'input shape: {input_array.shape}')
        model3d = UNetModel(
            input_array.shape[1:],
            nb_unet_levels=4,
            supervised=False,
            spacetime_ndim=3,
        )
        result = model3d.predict([input_array, input_array])
        assert result.shape == input_array.shape
        assert result.dtype == input_array.dtype
        # Verify model produces non-trivial output
        assert not np.allclose(result, 0)


def test_thin_masking_3D():
    np.random.seed(42)
    for i in range(3):
        input_array = np.random.rand(1, 2 + i, 64, 64, 1).astype(np.float32)
        print(f'input shape: {input_array.shape}')
        model3d = UNetModel(
            input_array.shape[1:],
            nb_unet_levels=4,
            supervised=False,
            spacetime_ndim=3,
        )
        result = model3d.predict([input_array, input_array])
        assert result.shape == input_array.shape
        assert result.dtype == input_array.dtype
        # Verify model produces non-trivial output
        assert not np.allclose(result, 0)
