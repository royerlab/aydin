...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.custom_rot90 = CustomRot90(spacetime_ndim) # TODO: assign correct custom module here for both 2d and 3d
        self.conv_with_batch_norm = ConvWithBatchNorm(spacetime_ndim)
        self.pooling_down = PoolingDown2D(shiftconv, pooling_mode) if spacetime_ndim == 2 else PoolingDown3D(shiftconv, pooling_mode)
        self.upsampling = UpSampling2D(shiftconv, upsampling_mode) if spacetime_ndim == 2 else UpSampling3D(shiftconv, upsampling_mode)
