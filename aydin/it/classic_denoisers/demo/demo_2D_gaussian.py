"""Demo script for 2D Gaussian filter denoising with auto-calibration."""
...
def demo_gaussian(image, display=True):
    """Denoise a 2D image using auto-calibrated Gaussian filtering."""
    ...
    if display:
        ...
        plt.savefig(os.path.join(_DEMO_RESULTS, 'classic_gaussian_2D.png'))
...
if __name__ == "__main__":
    ...
    demo_gaussian(newyork_image)
    characters_image = characters()
    demo_gaussian(characters_image)
    pollen_image = pollen()
    demo_gaussian(pollen_image)
    lizard_image = lizard()
    demo_gaussian(lizard_image)
    dots_image = dots()
    demo_gaussian(dots_image)
    camera_image = camera()
    demo_gaussian(camera_image)
