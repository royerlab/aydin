"""Demo script for 2D bilateral filter denoising with auto-calibration."""
...
from aydin.it.classic_denoisers.bilateral import calibrate_denoise_bilateral
...
def demo_bilateral(image, display=True):
    """Denoise a 2D image using auto-calibrated bilateral filtering."""
    ...
    start = time.time()
    function, parameters, memreq = calibrate_denoise_bilateral(noisy)
    ...
    print("bilateral denoised:", psnr_denoised, ssim_denoised)
    ...
    if display:
        ...
        plt.savefig(os.path.join(_DEMO_RESULTS, 'classic_bilateral_2D.png'))
...
if __name__ == "__main__":
    ...
    demo_bilateral(newyork_image)
    characters_image = characters()
    demo_bilateral(characters_image)
    pollen_image = pollen()
    demo_bilateral(pollen_image)
    lizard_image = lizard()
    demo_bilateral(lizard_image)
    dots_image = dots()
    demo_bilateral(dots_image)
    camera_image = camera()
    demo_bilateral(camera_image)
