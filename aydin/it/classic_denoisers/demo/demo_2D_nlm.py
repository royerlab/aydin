"""Demo script for 2D non-local means (NLM) denoising with auto-calibration."""
...
from aydin.it.classic_denoisers.nlm import calibrate_denoise_nlm
...
def demo_nlm(image, display=True):
    """Denoise a 2D image using auto-calibrated non-local means filtering."""
    ...
    start = time.time()
    function, parameters, memreq = calibrate_denoise_nlm(noisy, patch_size=5)
    ...
    print("nlm denoised:", psnr_denoised, ssim_denoised)
    ...
    if display:
        ...
        plt.savefig(os.path.join(_DEMO_RESULTS, 'classic_nlm_2D.png'))
...
if __name__ == "__main__":
    ...
    demo_nlm(newyork_image)
    characters_image = characters()
    demo_nlm(characters_image)
    pollen_image = pollen()
    demo_nlm(pollen_image)
    lizard_image = lizard()
    demo_nlm(lizard_image)
    dots_image = dots()
    demo_nlm(dots_image)
    camera_image = camera()
    demo_nlm(camera_image)
