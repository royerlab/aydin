"""Demo of Noise2Self denoising with a linear (Huber) regressor on 2D images.

Trains an ``ImageTranslatorFGR`` with ``LinearRegressor`` in Huber mode
on multiple standard test images and evaluates PSNR/SSIM.
"""
...
def demo(image, name):
    ...
    plt.savefig(os.path.join(_DEMO_RESULTS, f'n2s_lin_2D_{name}.png'))
