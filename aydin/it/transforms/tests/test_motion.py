"""Tests for the motion stabilisation transform."""
...
def test_phase_correlation():
    """Test that phase correlation detects known integer shifts."""
    a = camera()
...
def test_measure_shifts_and_transform():
    """Test shift measurement and shift transform correction on a stack."""
...
def test_correct_uncorrect():
    """Test motion correction preprocess/postprocess round-trip preserves data."""
    shifts = tuple((5 * i, int(0.5 * i * i)) for i in range(10))
...
def add_noise(image, intensity=4, variance=0.4):
    """Add Poisson and Gaussian noise to an image.
    ...
    intensity : float, optional
        Poisson noise intensity scaling factor. None to skip.
